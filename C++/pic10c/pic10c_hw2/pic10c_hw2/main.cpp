...
int main()
{
    /* A few suggestions in the begining:
       (1) always implement a non-template version first to make sure your code works by itself without the interface of templates; try different edge cases and examples to examine your code's correctness.
       (2) test the following cases one by one, i.e., testing constructors, empty heaps, push, top, etc. one by one; in your early state of testing, do not have an int main that contains commands that involve all function calls.
    */
